...
It first type checks the expression which was created by the reader (lines 4 and 5), and if it succeeds, it creates a \Racket{} module that binds several names and returns the value of reduced expression.
...
It can be done using the \texttt{(datum->syntax ctx sym)} function which takes any syntax object and creates a new one from the \texttt{sym}, inheriting the scope from the \texttt{ctx}.
...
The module defines variables containing the exression and its type, functions which can trace the reduction of the expression and functions that calculate the final value.
If the term did not typecheck, the expander raises an exception.
\begin{listing}[H]

...
\begin{listing}[H]
  \inputminted[firstline=12, lastline=19]{Racket}{../algeff/main.rkt}
  \caption{The reader}
  \label{lst:the-reader}
\end{listing}
The expander depicted in \autoref{lst:the-expander} creates a simple \Racket{} module which calls the helper function \texttt{run} passing it the syntax object.
This function first infers the type of an expression, and if it succeeds reduces the term using both reduction relation and abstract machine.
\begin{listing}[H]
  \inputminted[firstline=22, lastline=24]{Racket}{../algeff/main.rkt}
